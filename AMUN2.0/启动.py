...
def handle_option_1():    
    ...
    for i in range(X):  
        for j in range(X):  
            k = j * i  # 这个变量在循环内计算但没有被使用，可能是无效的  
        # 缩进修复，并确保 print 语句在正确的层级上  
        print('starting Amun...',  
              f'|{"█"*((i+1)*50//X):50}|',   
              f'{(i+1)*100//X}%',   
              end='\r')
    print('\nComplete！')
    print("Amun1.1@Amun system")
    print("Now,you can use the Amun System.")
    print("if you see [command]=you can input the command to use it.")
    print("if you need help please input help")
    subprocess.run(["python", "Amun System2.0.py"]) 
